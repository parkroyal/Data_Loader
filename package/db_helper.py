...
archivedb_to_server = archive_table_mapping[
    ~archive_table_mapping.mt4_server.isin(["vtm1report", "kcmreport"])
]
...
# %%
def get_data(
    query: str, db: str, schema: Union[str, List], db_type: str = "mysql_db"
) -> pd.DataFrame:
    """
    ...
    if "server" in df.columns:
        conditions = [(df.server.eq("mt5_vfx_live")), (df.server.eq("mt5_pug_live"))]
...
def get_sync_data(
    ...
    # Archive
    if do_archive:
        archive_servers = archive_table_mapping[
            archive_table_mapping.mt4_server.isin(mt4_servers)
        ]["archive_server"]
    ...
    if mt5_servers is not None:
        for server in mt5_servers:
            ...
            temp = get_data(
                query, db="mt5_with_archive", schema=server, db_type="maria_db"
            )
            df.append(temp)
    df = pd.concat(df)
...
def get_archive_data(
    ...
    for archive_server in archive_servers:
        query = query_archive.format(
            archive_server=archive_server, start_time=start_time, end_time=end_time
        )
    ...
    if "archive_server" in df.columns and "group" in df.columns:
        ...
        df = df.assign(
            server=lambda x: np.where(
                ...
                "mxtreport",
                np.where(
                    x.group.str.contains("VT") & x.archive_server.eq("mxt"),
                    "vtm1report",
                    x.mt4_server,
                ),
            )
    ...
    if "currency" in df.columns and "group" in df.columns:
        ...
        df = df.assign(
            currency=lambda x: np.where(
                pd.isna(x.currency) | x.currency.eq(""), x.group.str[-3:], x.currency
            )
        )
